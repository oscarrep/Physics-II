		...
		for (b2Fixture* f = b->GetFixtureList(); f; f = f->GetNext())
		{
			...
			if (App->input->GetMouseButton(SDL_BUTTON_LEFT) == KEY_DOWN)
			{
				...
				if (f->TestPoint({ PIXEL_TO_METERS(App->input->GetMouseX()), PIXEL_TO_METERS(App->input->GetMouseY()) }))
				{
					...
					App->renderer->DrawLine(mouse_joint->GetBodyB()->GetPosition().x, mouse_joint->GetBodyB()->GetPosition().y, mouse_joint->GetTarget().x, mouse_joint->GetTarget().y, 255, 0, 0, 80);
				}
			}
		}
	}
